...
public class IntroductionService extends AsyncTask<String, String, String> {

    CustomDialog customDialog;
    Activity activity;
    Interface.OnIntroduction onIntroduction;
    String Result, Language;

    public IntroductionService(Activity activity, Interface.OnIntroduction onIntroduction, String language) {
    ...
    protected String doInBackground(String... strings) {
        ...
        try {
            ...
            Form form = new Form()
                    .add(Constants.action, Constants.gettutorial)
                    .add(Constants.language, Language)
                    .add(Constants.appid, GlobalClass.Riyadh);

            Request request = Bridge
                    .post(Constants.URL + "/" + Constants.TutorialProcess)
                    .body(form)
                    .request();
        ...
        try {
            if (!s.equals("Error")) {
                ...
                if (jsonObject.getInt("status") == Constants.Success) {
                    onIntroduction.onIntroduction(jsonObject);
                } else {
